  ...
  return (
    <Show when={!skill().hidden}>
      <div class="skill-box figure">
        <div class="skill-type">{TYPE_TAG_TEXT_MAP[language][skill().type]}</div>
        {skill().playCost && <Cost type="skill" cost={skill().playCost} />}
        <div class="skill-icon" style={{ 'mask-image': `url("${skill().icon ? `/images/${skill().icon}.png` : MISSING_ICONS_URL[skill().id]}")` }} />
